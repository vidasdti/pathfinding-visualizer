...
class MapEditor(QGraphicsView):
    ...
    def build_graph(self):
        """
        Builds adjacency list representation of the undirected graph.
        """
        g = {}
        for e in self.edges:
            # main direction
            ...
            # reverse
            g.setdefault(e["to"], []).append((e["from"], e))
        return g
    ...
    def edge_info(self, a, b):
        """
        Returns (length, traffic, cost) for edge a -> b and b -> a.
        """
        for e in self.edges:
            if (
    ...
    def highlight(self, path):
        ...
        for i in range(len(path) - 1):
            for e in self.edges:
                if (
                    (e["from"] == path[i] and e["to"] == path[i + 1]) or
    ...
    def show_best(self):
        ...
        for i in range(len(self.best_path) - 1):
            for e in self.edges:
                if (
                    (e["from"] == self.best_path[i] and e["to"] == self.best_path[i + 1]) or
                    (e["to"] == self.best_path[i] and e["from"] == self.best_path[i + 1])
                    ):
    ...
    def keyPressEvent(self, e):
        if e.key() == Qt.Key_A:
            ...
            if self.current_algorithm == "UCS":
                self.ucs(self.start_node, self.goal_node)
        ...
        if self.alg_menu:
            if e.key() == Qt.Key_Up:
                self.alg_index = (self.alg_index - 1) % len(self.algorithms)
                self.update_algorithm_highlight()
                ...
                self.current_algorithm = self.algorithms[self.alg_index]
                print(f"✅ Algorithm set to {self.current_algorithm}")
                self.hide_algorithm_menu()
